...
vcf/%.somatic_indels.vcf : $(foreach type,$(INDEL_TYPES),vcf/%.$(type).uvcf.vcf)
	$(call RUN,-s 24G -m 48G -c -w 7200,"$(MERGE_INDEL_VCF) $^ | $(VCF_SORT) $(REF_DICT) - > $@.tmp && $(call VERIFY_VCF,$@.tmp,$@)")
...
chr_uvcf/%.$1.uvcf : chr_vcf/%.$1.vcf
	$$(call CHECK_UVCF,$$(call RUN,-s 24G -m 48G -c -w 7200,"$$(UPS_INDEL) $$(REF_FASTA) $$< $$(@D)/$$*.$1 -hd=true"))
endef
...
vcf/%.uvcf.vcf : vcf/%.vcf uvcf/%.uvcf
	$(call CHECK_VCF,$(call RUN,-s 24G -m 48G -w 7200,"$(MERGE_UVCF_VCF) $(<<) $(<) > $@"))
